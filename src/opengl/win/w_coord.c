...
char * text_maps[ATOM_MAPS] = {i18n("Atomic Species"),
                               i18n("Total"),
                               i18n("Partial"),
                               i18n("Fragment(s)"),
                               i18n("Molecule(s)")};
...
GtkWidget * coord_tab (glwin * view, int geo, int poly)
{
  ...
  GtkWidget * wb;
  GtkWidget * but;
  gchar * coln[15] = {" ", i18n("Color"), i18n("Show"), i18n("Label"), i18n("Pick"), i18n("Alpha"),
                           i18n("Color <sup>*</sup>"), i18n("Show <sup>**</sup>"), i18n("Label <sup>**</sup>"), i18n("Pick <sup>**</sup>"),
                           i18n("Ring(s) size"),
                           i18n("Chain(s) size"), i18n("Show <sup>*</sup>"), i18n("Label <sup>*</sup>"), i18n("Pick <sup>*</sup>")};
  ...
  if (geo > 3 && ! poly)
  {
    ...
    if (geo < 9)
    {
      add_box_child_start (GTK_ORIENTATION_VERTICAL, box, markup_label(_("  * for the polyhedra only"), -1, -1, 0.0, 0.5), FALSE, FALSE, 5);
      add_box_child_start (GTK_ORIENTATION_VERTICAL, box, markup_label(_("** affect all atoms that belong to ring(s) of this size"), -1, -1, 0.0, 0.5), FALSE, FALSE, 0);
    }
    else
    {
      add_box_child_start (GTK_ORIENTATION_VERTICAL, box, markup_label(_("* affect all atoms that belong to chain(s) of this size"), -1, -1, 0.0, 0.5), FALSE, FALSE, 0);
    }
  }
  ...
  for (i=0; i<5-2*l-geo/9; i++)
  {
    ...
    str = g_strdup_printf ("<b>%s</b>", _(coln[j]));
    ...
    for (i=0; i < m; i++)
    {
      ...
      for (j=0 ; j < this_proj -> coord -> ntg[geo][i]; j++, k++)
      {
        if (add_geo(poly, this_proj, geo, i, j))
        {
          ...
          {
            str = g_strdup_printf (_("%d atoms"), this_proj -> coord -> geolist[geo][i][j]);
          }
...
void set_this_frag_mol_color (gpointer data, GtkTreePath * path)
{
  ...
  coord_edition * coord = opengl_project -> modelgl -> coord_win;
  gtk_tree_model_get (GTK_TREE_MODEL(coord -> frag_mol_model[g-2]), & iter, 0, & i, -1);
  gchar * ctitle[2] = {i18n("Fragment"), i18n("Molecule")};
  gchar * str = g_strdup_printf (_("%s N°%d color"), _(ctitle[g-2]), i);
...
GtkWidget * create_frag_mol_tree (project * this_proj, int geo)
{
  ...
  gchar * ctitle[5]={i18n("Id."), i18n("Show"), i18n("Color"), i18n("Label"), i18n("Pick")};
  ...
  for (i=0; i<5; i++)
  {
    ...
    frag_mol_col[i] = gtk_tree_view_column_new_with_attributes (_(ctitle[i]), frag_mol_cell[i], ctype[i], i, NULL);
...
GtkWidget * create_frag_mol_search (project * this_proj, int geo)
{
  ...
  add_box_child_start (GTK_ORIENTATION_HORIZONTAL, hbox, markup_label(_(search_item[geo-2]), 100, -1, 0.0, 0.5), FALSE, FALSE, 20);
  ...
  add_box_child_start (GTK_ORIENTATION_VERTICAL, frag_mol_search, markup_label(_("<b>Search result(s)</b>"), 200, -1, 0.5, 0.5), FALSE, FALSE, 10);
...
GtkWidget * fragmol_tab (glwin * view, int geo)
{
  ...
  gchar * coln[5] = {i18n("Id."), i18n("Color"), i18n("Show"), i18n("Label"), i18n("Pick")};
...
GtkWidget * param_tab (glwin * view)
{
  ...
  GtkWidget * lab = markup_label (_("<b>Atom(s) and bond(s) color map:</b>"), -1, -1, 0.0, 0.5);
  ...
  for (i=0; i<ATOM_MAPS; i++)
  {
    ...
    {
      view -> cmap[j] = i;
      combo_text_append (color_box, g_strdup_printf (_("%s coordination(s)"), _(text_maps[i])));
      j ++;
    }
    else if (i == 5 && get_project_by_id(view -> proj) -> force_field[0])
    {
      view -> cmap[j] = i;
      combo_text_append (color_box, _("Force field (DL_POLY)"));
      j ++;
    }
    else if (i == 6 && view -> custom_map != NULL)
    {
      view -> cmap[j] = i;
      combo_text_append (color_box, _("Custom"));
      j ++;
    }
    else if (i == 0 || (i == 3 && view -> adv_bonding[0]) || (i == 4 && view -> adv_bonding[1]))
    {
      view -> cmap[j] = i;
      combo_text_append (color_box, _(text_maps[i]));
      j ++;
    }
  }
  l = 0;
  ...
  hbox = create_hbox (0);
  add_box_child_start (GTK_ORIENTATION_VERTICAL, vbox, hbox, FALSE, TRUE, 25);
  lab = markup_label (_("<b>Polyhedra color map:</b>"), -1, -1, 0.0, 0.5);
  add_box_child_start (GTK_ORIENTATION_HORIZONTAL, hbox, lab, FALSE, FALSE, 20);
  hbox = create_hbox (0);
  add_box_child_start (GTK_ORIENTATION_VERTICAL, vbox, hbox, FALSE, FALSE, 0);
  ...
  for (i=0; i<POLY_MAPS; i++)
  {
    ...
    {
      combo_text_append (color_box, _("Use atom(s) custom map"));
    }
  }
  l = 0;
  for (k=0; k<j; k++)
  ...
  add_box_child_start (GTK_ORIENTATION_VERTICAL, vbox, markup_label(_("\n<b>Display polyhedra at the edges of the model box\n"
                                                                      "using the PBC even if the clones are not shown:</b>"), -1, -1, 0.5, 0.5), FALSE, FALSE, 15);
  GtkWidget * cloned_p = check_button (_("Cloned polyhedra"), -1, 40, view -> anim -> last -> img -> cloned_poly,
  ...
  add_box_child_start (GTK_ORIENTATION_VERTICAL, vbox, markup_label(_("This is automatic if the clones are shown.\n"
                                                                      "The clones are the replica of the atoms linked by PBC."), -1, -1, 0.5, 0.5), FALSE, FALSE, 5);
  return vbox;
}
...
GtkWidget * advanced_coord_properties (glwin * view, int page)
{
  gchar * str = g_strdup_printf (_("Environments configuration - %s"), get_project_by_id(view -> proj)->name);
  ...
  gtk_notebook_append_page (GTK_NOTEBOOK(view -> coord_win -> notebook), param_tab (view), gtk_label_new (_("Parameters")));
  gtk_notebook_append_page (GTK_NOTEBOOK(view -> coord_win -> notebook), coord_tab (view, 0, 0), markup_label(_("Total coordination(s) <b>[TC]</b>"), -1, -1, 0.0, 0.5));
  gtk_notebook_append_page (GTK_NOTEBOOK(view -> coord_win -> notebook), coord_tab (view, 1, 0), markup_label(_("Partial coordination(s) <b>[PC]</b>"), -1, -1, 0.0, 0.5));
  ...
  {
    gtk_notebook_append_page (GTK_NOTEBOOK(view -> coord_win -> notebook), coord_tab (view, 9, 0), markup_label(_("Chain(s)"), -1, -1, 0.0, 0.5));
  }
  gtk_notebook_append_page (GTK_NOTEBOOK(view -> coord_win -> notebook), coord_tab (view, 0, 1), markup_label(_("Polyhedra from <b>TC</b>"), -1, -1, 0.0, 0.5));
  gtk_notebook_append_page (GTK_NOTEBOOK(view -> coord_win -> notebook), coord_tab (view, 1, 1), markup_label(_("Polyhedra from <b>PC</b>"), -1, -1, 0.0, 0.5));
  if (view -> rings)
  {
    for (i=0; i<5; i++)
    {
      if (view -> ring_max[i])
      {
        str = g_strdup_printf (_("Polyhedra from <b>%s</b>"), rings_short[i]);
  ...
  {
    gtk_notebook_append_page (GTK_NOTEBOOK(view -> coord_win -> notebook), fragmol_tab (view, 2), markup_label (_("Fragment(s)"), -1, -1, 0.0, 0.5));
  }
  if (view -> adv_bonding[1])
  {
    gtk_notebook_append_page (GTK_NOTEBOOK(view -> coord_win -> notebook), fragmol_tab (view, 3), markup_label (_("Molecule(s)"), -1, -1, 0.0, 0.5));
  }
  ...
  if (view -> rings)
  {
    for (i=0; i<5; i++)
    {
      if (view -> ring_max[i])
      {
        str = g_strdup_printf (_("Isolated ring(s) from <b>%s</b>"), rings_short[i]);
  ...
  if (view -> chains && view -> chain_max)
  {
     gtk_notebook_append_page (GTK_NOTEBOOK(view -> coord_win -> notebook), chains_tab (view), markup_label(_("Isolated chain(s)"), -1, -1, 0.0, 0.5));
  }

  GtkWidget * hbox = create_hbox (5);
  add_box_child_start (GTK_ORIENTATION_VERTICAL, vbox, hbox, TRUE, FALSE, 0);
  GtkWidget * but = create_button (_("Close"), IMG_STOCK, FCLOSE, -1, -1, GTK_RELIEF_NORMAL, G_CALLBACK(close_coord), view);
...
G_MODULE_EXPORT void coord_properties (GtkWidget * widg, gpointer data)
{
  ...
  {
    show_warning (_("Error impossible to display the environments window !"), view -> win);
  }
}
